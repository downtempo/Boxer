...
| **A17** 🟢 (Blocked: hardware) | **Pace Metal presentation to the DOS video mode refresh rate.** Carry the active mode's native rate from DOSBox `render.src.fps` as a per-frame hint named `BXVideoFrame.videoModeFramesPerSecond` (`lround` + clamp, `0` = unknown, assigned unconditionally in `prepareForOutputSize:` like `containsText`). Then pace the renderer to that hint. Choose the mechanism (`MTKView.preferredFramesPerSecond` or `presentDrawable:afterMinimumDuration:`) by measurement, not assumption: a fixed display above ~70 Hz is required, which is unavailable locally. The `preferredFramesPerSecond` write must marshal to main because the frame path runs on the emulation thread in the default configuration; any cross-thread pacing scalar must use an atomic or main-thread handoff so Thread Sanitizer remains clean. Portable plumbing and the winning mechanism land together. Do not merge a dead accessor with no consumer. `CAMetalDisplayLink` is out of scope; see A27. | [Boxer/Rendering/](Boxer/Rendering), [BXVideoHandler.mm](Boxer/BXVideoHandler.mm), [BXMetalRenderingView.m](Boxer/Metal%20Rendering/BXMetalRenderingView.m) | Runtime (blocked) |
...
| **A22** 🟢 (Experiment, later) | **Evaluate macOS Game Mode.** Do not land `LSSupportsGameMode = true` by default until it proves it improves Boxer enough to outweigh the extra system presence. The concern is UX as much as performance: Game Mode can surface distracting OS-level affordances in other emulators, and Boxer should stay quiet unless the benefit is clear. When revisited late in the roadmap, test a plist-only branch in real fullscreen gameplay, compare frame pacing / power behavior, observe any banners, overlays, or controller affordances, and only then decide whether to add the key to both app plists. Do not link `GameController.framework` here; framework adoption belongs with C0's GameController migration. | [Info.plist](Info.plist), [Standalone/Boxer Standalone-Info.plist](Standalone/Boxer%20Standalone-Info.plist) | Runtime experiment, defer |
...
| **A27** 🔴 | **macOS 26-only: drive Metal presentation from `CAMetalDisplayLink`.** Replace the free-running `paused = NO` MTKView loop with a `CAMetalDisplayLink`-driven present path (macOS 14+, unconditional at the `macos26` deployment floor, no `CVDisplayLink` fallback), consuming the same `BXVideoFrame.videoModeFramesPerSecond` seam A17 introduces. This is also the correct place to introduce a synchronized handoff for the pre-existing `_texture` / `_filterChain` cross-thread access. Land after A17 so the upstream cherry-pick range excludes it. Fork-specific narrowing, not for upstream. | [BXMetalRenderingView.m](Boxer/Metal%20Rendering/BXMetalRenderingView.m), [Boxer/Rendering/](Boxer/Rendering) | Fork-specific Runtime |
...
| A17 | Blocked (hardware) | mechanism choice and judder claim need a fixed >70 Hz display; portable plumbing specified |
...
| A22 | Experiment, later | UX and power impact must be proven after the rest of the runtime work; no default-on plist change for now |
| A27 🔴 | Follow-up | `CAMetalDisplayLink` presentation driver for `macos26`, after A17 |
...
| B14 🟢 | **Pre-prompt explainers for TCC permissions.** A10 and A11 added Info.plist usage strings so the *OS dialog itself* has reasonable text, but there's no Boxer-side context shown *before* the OS prompt fires for the first time. Add a one-time, dismissible Boxer sheet at the call sites where TCC-protected APIs first fire - at minimum: first gamebox import that crosses into `~/Documents`/`~/Desktop`/`~/Downloads`, first removable-volume mount (CD/USB), first gamepad connection (Input Monitoring). The sheet should explain (1) what permission is about to be asked, (2) what happens if the user clicks Deny in the OS dialog, (3) how to grant it later via Settings. Boxer already has the equivalent pattern for Accessibility via [hotkeyWarningAlert](Boxer/Application%20Delegate/BXBaseAppController+BXHotKeys.m:341); B14 extends that pattern to filesystem/input prompts. As part of the same permission-handoff work, replace old `.prefPane` paths and `com.apple.systempreferences` ScriptingBridge usage in the Accessibility prompt flow with a modern System Settings URL handoff. A10 and A11 have landed, so this item is unblocked. | [BXSession.m](Boxer/BXSession.m) (drive imports), [ADBHIDMonitor.m](Other%20Sources/ADBToolkit/ADBHIDMonitor.m) (first HID enumeration), [BXBaseAppController+BXHotKeys.m](Boxer/Application%20Delegate/BXBaseAppController+BXHotKeys.m) (Accessibility handoff), new explainer view/sheet | Small (copy + prompt flow) |
...
| Item(s) | Readiness | Basis |
|---------|-----------|-------|
| B14, B15 | Ready | B14 is unblocked by A10/A11 landing and includes prompt-flow handoff code; B15 is copy/locales |
...
| `build/signing-and-notarization` | A8, A9, A13 | Complete. Hardened Runtime on (Boxer + Standalone), Standalone entitlements wired. Failure mode: codesign / notarization. Land first: A8 unlocks entitlement enforcement. Must include or integrate with populated JIT/library-validation entitlements, not the empty `maddsV2` stub. |
| `build/privacy-usage-strings` | A10, A11 | Complete. TCC usage descriptions in Boxer and Standalone app plists. Failure mode: first-run permission UX. |
| `build/share-workspace-schemes` | A14 | Complete. Workspace schemes (unlocks command-line builds without the CI scheme). |
| `experiment/game-mode` | A22 | Deferred until late roadmap. Test `LSSupportsGameMode` in real fullscreen gameplay for frame pacing, power, and distracting system UI before deciding whether it should ship. |
| `perf/metal-frame-scheduling` | A17, A21 | Frame pacing + redraw-skip. Renderer-architecture changes; A17 is blocked on fixed >70 Hz hardware, A21 after design review. Runtime QA. A27 (`CAMetalDisplayLink`) is a separate `macos26/` branch after A17. |
| `macos26/perf/metal-display-link-presentation` | A27 🔴 | `CAMetalDisplayLink` presentation driver; after A17. Never upstreamed. |
...
- All TCC plist keys present and `ENABLE_HARDENED_RUNTIME = YES` consistently.
- Universal SDL frameworks slimmed to arm64.
- 60 s gameplay frame variance: A17 target, gated on fixed >70 Hz hardware measurement. Do not assume the outcome; it may resolve as a judder fix, energy-only improvement, or no-op.
...
- Energy Impact in Activity Monitor stays "Low" / "Medium" during typical
  gameplay; Game Mode (A22) remains a later UX/performance experiment rather
  than an end-state requirement.
